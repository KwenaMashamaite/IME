...
namespace IME::Gui {
    WindowClosePopUpMenu::WindowClosePopUpMenu()
    ...
    void WindowClosePopUpMenu::draw(Window &renderTarget) {
        renderTarget.draw(*panel_);
    }

    void WindowClosePopUpMenu::hide() {
        if (!isHidden()){
            isHidden_ = true;
            panel_->hide();
        }
    ...
    void WindowClosePopUpMenu::show() {
        if (isHidden()){
            isHidden_ = false;
            panel_->show();
        }
    ...
    bool WindowClosePopUpMenu::isHidden() const {
        return isHidden_;
    }

    void WindowClosePopUpMenu::onClick(const std::string &button, Callback<> callback){
