...
void Gui::StackPanel::addElement(std::shared_ptr<UIElement> guiElement) {
    ...
    if (inserted) {
        if (size() == 1) {
            setDimensions({
                guiElement->getDimensions().width, 
                guiElement->getDimensions().height
            });
            ...
            switch (orientation_){
                case Orientation::Vertical:
                    guiElement->setPosition(
                        lastInsertedElement_->getPosition().x, 
                        lastInsertedElement_->getPosition().y
                        + lastInsertedElement_->getDimensions().height
                    );
                    newWidth = guiElement->getDimensions().width 
                               > lastInsertedElement_->getDimensions().width
                               ? guiElement->getDimensions().width 
                               : lastInsertedElement_->getDimensions().width;
                    setDimensions({
                        newWidth, 
                ...
                case Orientation::Horizontal:
                    ...
                    newHeight = guiElement->getDimensions().height 
                                > lastInsertedElement_->getDimensions().height
                                ? guiElement->getDimensions().height 
                                : lastInsertedElement_->getDimensions().height;
                    setDimensions({
